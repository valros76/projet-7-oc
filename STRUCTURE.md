...
│   │   ├── LoginForm.vue                 # Formulaire de connexion des utilisateurs
...
│   │   └── api.ts                        # Client HTTP global ($api) avec injection Bearer & Refresh auto
...
│   │   └── leads/
│   │       ├── index.get.ts              # Liste des leads (sécurisée par rôle)
...
│       └── auth.ts                       # Fonctions de signature, vérification des JWT et hachage bcrypt
...
├── nuxt.config.ts                        # Configuration globale du framework Nuxt 4 (modules, CSS, imports, alias)
